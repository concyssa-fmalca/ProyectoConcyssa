...
function llenarComboTipoRegistroFiltro(lista, idCombo, primerItem) {
    ...
    var cbo = document.getElementById(idCombo);
    if (cbo != null) cbo.innerHTML = contenido;
    $("#cboTipoRegistroFiltro").prop("selectedIndex", 3);
}
function ObtenerSemanas() {
    ...
    $.ajaxSetup({ async: false });
    $.post("/TipoRegistro/ObtenerSemanasXIdObraAnioIdTipoRegistro", { 'IdObra': ObraSemana, 'Anio': AnioSemana, 'IdTipoRegistro': 5 }, function (data, status) {
        if (validadJson(data)) {
...
function ObtenerReporte() {
    ...
    setTimeout(() => {
        $.ajaxSetup({ async: false });
        $.post("GenerarReporte", {
            'NombreReporte': 'InformeCajaChica', 'Formato': 'PDF', 'IdObra': varIdObraReporte, 'Anio': varAnioReporte, 'IdSemana': varIdSemanaReporte
