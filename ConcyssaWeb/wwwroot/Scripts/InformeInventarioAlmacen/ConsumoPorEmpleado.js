...
function llenarComboBaseFiltro(lista, idCombo, primerItem) {
    ...
    $("#cboBaseFiltro").prop("selectedIndex", 1).change();
...
function ObtenerReporte(Formato) {
    ...
    let TipoProducto = 2
    if ($("#radioMateriales").is(':selected')) {
        TipoProducto = 1
    }
    ...
    setTimeout(() => {
        ...
        $.ajaxSetup({ async: false });
        $.post("GenerarReporteConsumoEmpleado", { 'NombreReporte': 'RPT_ConsumoxEmpleado', 'Formato': Formato, 'IdEmpleado': $("#cboEmpleado").val(), 'IdObra': $("#cboObraFiltro").val(), 'FechaInicio': $("#txtFechaInicio").val(), 'FechaFin': $("#txtFechaFin").val(), 'IdTipoProducto': TipoProducto }, function (data, status) {
            let datos;
            if (validadJson(data)) {
                if (Formato == 'excel') {
                    try {
                        ...
                        link.download = `ConsumoEmpleado.xlsx`;
                        link.click();
                        Swal.close()
                    } catch (e) {
...
function ObtenerEmpleadosxIdCuadrilla() {
    $.ajaxSetup({ async: false });
    $.post("/Empleado/ObtenerEmpleadosPorIdBase", { 'IdBase': $("#cboBaseFiltro").val() }, function (data, status) {
        try {
            let empleados = JSON.parse(data);
            llenarComboEmpleados(empleados, "cboEmpleado", "TODOS")
        } catch (e) {
            $("#cboEmpleado").html("<option value='0'>TODOS</option>")
        }
    });
}
...
function llenarComboEmpleados(lista, idCombo, primerItem) {
    ...
    for (var i = 0; i < nRegistros; i++) {
        ...
        if (lista.length > 0) { contenido += "<option value='" + lista[i].IdEmpleado + "'>" + lista[i].RazonSocial.toUpperCase() + "</option>" }
        else { }
    }
    var cbo = document.getElementById(idCombo);
    if (cbo != null) cbo.innerHTML = contenido;
