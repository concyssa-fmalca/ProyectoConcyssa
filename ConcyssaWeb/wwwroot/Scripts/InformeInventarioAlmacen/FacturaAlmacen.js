...
function ObtenerReporte() {
    ...
    $.ajaxSetup({ async: false });
    $.post("GenerarReporteFacturaAlmacen", { 'NombreReporte': 'FacturaMueveStock', 'Formato': 'PDF', 'IdAlmacen': $("#cboAlmacenFiltro").val(), 'FechaInicio': $("#txtFechaInicio").val(), 'FechaFin': $("#txtFechaFin").val() }, function (data, status) {
        let datos;
        if (validadJson(data)) {
            let datobase64;
