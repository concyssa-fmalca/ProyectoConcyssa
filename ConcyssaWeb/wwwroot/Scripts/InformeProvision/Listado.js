...
function ObtenerReporte() {
    ...
    let varNumeroSemanaReporte = $("#cboSemana").val();
    $.ajaxSetup({ async: false });
    $.post("GenerarReporte", {
        'NombreReporte': 'InformeProvision', 'Formato': 'PDF', 'IdObra': varIdObraReporte, 'Anio' : varAnioReporte,'NumeroSemana':varNumeroSemanaReporte}, function (data, status) {
        let datos;
        if (validadJson(data)) {
            let datobase64;
