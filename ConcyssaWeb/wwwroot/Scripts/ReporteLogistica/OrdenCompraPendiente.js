function GenerarReporteOcPendiente(Tipo) {
    ...
    setTimeout(() => {
        ...
        if (Tipo == 'excel') {
            ...
            $.post("GenerarReporteOcPendiente", { 'NombreReporte': 'OcPendiente', 'Formato': 'excel', 'IdProveedor': IdProveedor, 'IdBase': IdBase, 'IdObra': IdObra, 'FechaInicial': FechaInicial, 'FechaFinal': FechaFinal }, function (data, status) {
                let datos;
                if (validadJson(data)) {
                    window.open("http://192.168.0.209/Anexos/ExcelReporte/OcPendiente.xlsx", '_blank', 'noreferrer');
                    Swal.fire(
                        'Correcto',
                        'Excel Generado',
                        'success'
