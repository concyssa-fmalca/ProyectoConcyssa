function CargarObras() {
    ...
    $.post("/Obra/ObtenerObra", { estado: 1 }, function (data, status) {
        ...
        llenarComboObras(obras, "IdObra", "Seleccione")
    });
}

function llenarComboObras(lista, idCombo, primerItem) {
    ...
    for (var i = 0; i < nRegistros; i++) {
        ...
        if (lista.length > 0) { contenido += "<option value='" + lista[i].IdObra + "'>" + lista[i].DescripcionCorta + "</option>"; }
        else { }
    }
...
function GenerarReporteStockSinConsumo() {
    let IdObra = $("#IdObra").val();
    let IdTipo = $("#IdTipo").val();
    let Meses = $("#MesesSinConsumo").val();
    let respustavalidacion = "";
    $.ajaxSetup({ async: false });
    $.post("GenerarReporteStockSinConsumo", { 'NombreReporte': 'StockSinConsumo', 'Formato': 'PDF', 'IdObra': IdObra, 'IdTipo': IdTipo, 'Meses': Meses }, function (data, status) {
        let datos;
        if (validadJson(data)) {
            let datobase64;
