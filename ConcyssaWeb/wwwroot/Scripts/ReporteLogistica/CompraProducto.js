function GenerarReporteCompraProducto(Tipo) {
    ...
    setTimeout(() => {
        if (Tipo == 'excel') {
            ...
            $.post("GenerarReporteCompraProducto", { 'NombreReporte': 'ComprasPorProducto', 'Formato': 'excel', 'IdArticulo': IdArticulo }, function (data, status) {
                let datos;
                if (validadJson(data)) {
                    window.open("http://192.168.0.209/Anexos/ExcelReporte/ComprasPorProducto.xlsx", '_blank', 'noreferrer');
                    Swal.fire(
                        'Correcto',
                        'Excel Generado',
                        'success'
            ...
            $.ajaxSetup({ async: false });
            $.post("GenerarReporteCompraProducto", { 'NombreReporte': 'ComprasPorProducto', 'Formato': 'PDF' , 'IdArticulo': IdArticulo }, function (data, status) {
                let datos;
                if (validadJson(data)) {
                    let datobase64;
