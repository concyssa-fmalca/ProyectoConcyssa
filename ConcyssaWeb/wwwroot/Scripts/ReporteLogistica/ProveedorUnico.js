function GenerarReporteProveedorUnico() {
    let FechaIni = $("#FechaInicio").val();
    let FechaFin = $("#FechaFin").val();
    let respustavalidacion = "";
    $.ajaxSetup({ async: false });
    $.post("GenerarReporteProveedorUnico", { 'NombreReporte': 'ProveedorUnicoProducto', 'Formato': 'PDF', 'FechaIni': FechaIni, 'FechaFin': FechaFin }, function (data, status) {
        let datos;
        if (validadJson(data)) {
            let datobase64;
