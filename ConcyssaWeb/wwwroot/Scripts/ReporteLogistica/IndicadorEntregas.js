...
function GenerarIndicadorEntregas(Tipo) {

    let AnoInput = +$("#Input").val();

    let Anno = AnoInput;
    let Detalle = false;//true;
    ...
    setTimeout(() => {
        if (Tipo == 'excel') {
            ...
            $.post("GenerarReporteIndicadorEntregas", { 'NombreReporte': 'IndicadorEntregas', 'Formato': 'excel', 'Anno': Anno, 'Detalle': Detalle }, function (data, status) {
                let datos;
                if (validadJson(data)) {
                    window.open("http://192.168.0.209/Anexos/ExcelReporte/RegistroAnual" + Anno + ".xlsx", '_blank', 'noreferrer');
                    Swal.fire(
                        'Correcto',
                        'Excel Generado',
                        'success'
            ...
            $.ajaxSetup({ async: false });
            $.post("GenerarReporteIndicadorEntregas", { 'NombreReporte': 'IndicadorEntregas', 'Formato': 'PDF', 'Anno': Anno, 'Detalle': Detalle }, function (data, status) {
                let datos;
                if (validadJson(data)) {
                    let datobase64;
