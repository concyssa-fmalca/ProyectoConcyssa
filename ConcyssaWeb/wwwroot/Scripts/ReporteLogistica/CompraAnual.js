...
function GenerarReporteAnual(Tipo) {
    ...
    setTimeout(() => {
        if (Tipo == 'excel') {
            ...
            $.post("GenerarReporteCompraAnual", { 'NombreReporte': 'ComprasAnualesCR', 'Formato': 'excel', 'Anno': Anno, 'Detalle': Detalle }, function (data, status) {
                let datos;
                if (validadJson(data)) {
            ...
            $.ajaxSetup({ async: false });
            $.post("GenerarReporteCompraAnual", { 'NombreReporte': 'ComprasAnualesCR', 'Formato': 'PDF', 'Anno': Anno, 'Detalle': Detalle }, function (data, status) {
                let datos;
                if (validadJson(data)) {
                    let datobase64;
