...
function GenerarReporte(Formato) {
    ...
    setTimeout(() => {
        ...
        $.post("GenerarReporteTransferencia", { 'NombreReporte':'ReporteTransferencias','Formato': Formato, 'IdObra': IdObra, 'IdAlmacen': IdAlmacen, 'IdTipoProducto': IdTipoProducto, 'FechaInicio': FechaInicio, 'FechaFin': FechaFin }, function (data, status) {
            let datos;
            if (validadJson(data)) {
                if (Formato == 'excel') {
                    try {
                        ...
                        link.download = `Transferencias.xlsx`;
                        link.click();
                        Swal.close()
                    } catch (e) {
