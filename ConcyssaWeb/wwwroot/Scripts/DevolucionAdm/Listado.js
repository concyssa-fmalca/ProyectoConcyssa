...
function Imprimir() {
    ...
    setTimeout(() => {
        ...
        $.ajaxSetup({ async: false });
        $.post('/DevolucionAdm/GenerarReporte', { 'IdBase': IdBase, 'IdObra': IdObra, 'FechaInicio': FechaInicio, 'FechaFin': FechaFin, 'EstadoDevolucion': EstadoSolicitud, 'SerieFiltro': SerieFiltro,'Formato':"pdf" }, function (data, status) {
            let datos;
            if (validadJson(data)) {
                let datobase64;
